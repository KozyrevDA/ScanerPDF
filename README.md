...
| 4. Фильтры и страницы | ✅ (+ импорт из галереи; reorder кнопками, drag-and-drop позже) |
| 5. PDF (A4/авто, поля, качество, share; экспорт из меню главного экрана) | ✅ |
| 6. OCR | Экран/прогресс/отмена/редактирование ✅; движок — заглушка `OcrEngine` |
| 7. AI | Экран анализа (саммари/реквизиты/договор), согласие, дисклеймер, mock + Ktor-клиент прокси, лимиты ✅ |
| 8. RuStore Billing | Заглушка за интерфейсом |
| 9. Релиз | R8-конфиг, detekt ✅; иконка/сплэш/финальные проверки — далее |

Инструментальные тесты: `composeApp/src/androidTest` — Room DAO (многостраничный
документ, восстановление, reorder-транзакция, каскадное удаление). Запуск:
`./gradlew :composeApp:connectedDebugAndroidTest` (нужен эмулятор/устройство).

Статический анализ: `./gradlew :composeApp:detekt` (конфиг `config/detekt/detekt.yml`).
